...
type ConductorJobOutputResponse = {
  ok?: boolean
  found?: boolean
  completed?: boolean
  outputPath?: string
...
async function fetchConductorJobOutput(
  jobId: string,
): Promise<ConductorJobOutputResponse> {
  const response = await fetch(
    `/api/conductor-output?jobId=${encodeURIComponent(jobId)}`,
  )
  const payload = (await response
    .json()
    .catch(() => ({}))) as ConductorJobOutputResponse
  if (!response.ok || payload.ok === false) {
    throw new Error(
      payload.error || `Failed to load conductor output for ${jobId}`,
    )
  }
  return payload
}
...
export function useConductorGateway() {
  ...
  const [orchestratorJobId, setOrchestratorJobId] = useState<string | null>(
    null,
  )
  ...
  const conductorOutputQuery = useQuery({
    queryKey: ['conductor', 'job-output', orchestratorJobId],
    queryFn: () => fetchConductorJobOutput(orchestratorJobId as string),
    enabled:
      Boolean(orchestratorJobId) &&
  ...
  useEffect(() => {
    const payload = conductorOutputQuery.data
    if (!payload?.completed || !payload.found) return
    ...
    if (finalResponse) {
      ...
      setPlanText((current) =>
        current.trim()
          ? current
          : 'Conductor job completed. The final cron output is shown below.',
      )
    }
  ...
  const sendMission = useMutation({
    mutationFn: async ({
      ...
      setOrchestratorJobId(result.jobId ?? null)
      ...
      setPlanText(
        result.triggerError
          ? `Orchestrator job created, but immediate trigger failed: ${result.triggerError}. Waiting for the cron runner...`
