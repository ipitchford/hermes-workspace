...
function getActivityLabel(member: CrewMember): {
  label: string
  value: string
} {
  if (member.lastSessionAt) {
    return {
      label: 'Last active',
      value: formatRelativeTime(member.lastSessionAt),
    }
  }

  if (member.profileKind === 'research_council') {
    if (!member.lastCouncilRunAt) {
      return { label: 'Council', value: 'Configured, not invoked' }
    }

    if (member.lastCouncilDryRun) {
      return {
        label: 'Council',
        value: `Dry-run only - ${formatRelativeTime(member.lastCouncilRunAt)}`,
      }
    }

    const status = member.lastCouncilStatus || 'run recorded'
    return {
      label: 'Council',
      value: `${status} - ${formatRelativeTime(member.lastCouncilRunAt)}`,
    }
  }

  return { label: 'Last active', value: 'Never' }
}
      ...
      >
        ...
        <div>
          <p className="text-[11px] text-[var(--theme-muted)]">
            {activity.label}:{' '}
            <span className="text-[var(--theme-text)]">{activity.value}</span>
          </p>
          {!member.lastSessionAt &&
            member.profileKind === 'research_council' &&
            member.lastCouncilQuestion && (
